...
# --- KEY CHANGE: Import the new, correct function for in-memory data ---
from src.data_processor import process_json_data
from src.pipelines import build_rag_pipeline, build_indexing_pipeline
from src.schemas import RAGResponse, QueryRequest, UpdateResponse, Category
...
# This is a performance optimization: the pipeline is loaded once on startup.
...
async def ask_question(request: QueryRequest):
    """
    Receives a user query and returns a structured answer from the RAG pipeline.
    """
...
async def update_articles_from_file(file: UploadFile = File(...)):
    """
    Accepts a JSON file upload, processes it using the modular data_processor, 
    and runs the indexing pipeline to update documents in Pinecone.
    """
    ...
    try:
        ...
        # --- KEY FIX: Call the function designed for raw JSON data ---
        ...
        # Build and run the indexing pipeline on the processed documents
