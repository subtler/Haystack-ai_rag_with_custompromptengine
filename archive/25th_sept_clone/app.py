...
from src.schemas import RAGResponse, QueryRequest, UpdateResponse, Article, Folder, Category
...
async def ask_question(request: QueryRequest):
    ...
    try:
        ...
        # --- KEY FIX: Provide the query to BOTH components that need it at the start ---
...
async def update_articles_from_file(file: UploadFile = File(...)):
    """
    Accepts a JSON file upload, processes it, and runs the indexing pipeline
    to update the documents in the Pinecone vector store.
    """
    ...
    try:
        ...
        docs_to_index = load_and_process_data(json_data)
